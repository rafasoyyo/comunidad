...
import { Avatar, Box, Button, Circle, FormControl, FormErrorMessage, FormLabel, HStack, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Radio, RadioGroup, Stack, StackDivider, Text, Textarea, useDisclosure, useRadio, useRadioGroup, VStack } from '@chakra-ui/react';

import Layout from '../components/common/layout';

import { ConfigInterface, ConfigContext } from '../services/config';
import { ArrowForwardIcon, CloseIcon } from '@chakra-ui/icons';
import EventService, { EventInterface } from '../services/events';
                    ...
                    overflowY="scroll">
                    {
                        props.displayEvents.map((event: EventInterface) => (
                            <Box
                                ...
                                w="100%">
                                <HStack>
                                    {event.backgroundColor && <Circle bg={event.backgroundColor} w="5" h="5" />}
                    ...
                    <ModalBody>
                        ...
                        <FormControl isRequired mt="2">
                            ...
                            <Textarea
                                ...
                                value={event.title}
                                onChange={(e) => setEvent({ ...event, title: (e.target.value).trim() })}></Textarea>
...
const EventsSelector = (props: {
    ...
    return (
        <RadioGroup onChange={onChange} defaultValue={value.name}>
            <Stack direction='row'>
                {options.map(option => (
                    <Radio key={option.name} value={option.name}>{t(option.name)}</Radio>
                ))}
            </Stack>
        </RadioGroup>
    )
